...
# Run from your Magento project root: bash claude-code-sdlc-workflow/magento-workflow/setup.sh
...
" 2>/dev/null
        if [ $? -eq 0 ]; then
            ...
            echo "Warning: Could not auto-update CLAUDE.md. Manually replace '## Magento Conventions' section."
        fi
    else
        echo "" >> CLAUDE.md
